...
const ContentCard = ({ content, recordView = true }) => {
  ...
  useEffect(() => {
    if (inView && recordView) {
      api.post(`/feed/view/${id}`).catch(err => {
  ...
  return (
    <Card ref={ref} className="mb-4 hover:shadow-md transition-shadow duration-200">
...
const PersonalizedFeed = () => {
  ...
  const [activeTab, setActiveTab] = useState(isAuthenticated ? 'recommended' : 'trending');
  ...
  const fetchRecommendedContent = async () => {
    try {
      ...
      if (response.data.success) {
        setRecommendedContent(response.data.recommendations);
      }
    } catch (error) {
  ...
  const fetchFollowingContent = async () => {
    try {
      ...
      if (response.data.success) {
        setFollowingContent(response.data.content);
      }
    } catch (error) {
